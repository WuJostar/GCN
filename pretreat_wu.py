...
CAPACITY = 256
INPUT_IMAGE_LEN = 384
ori_file_dir  = '/home/irisubuntu/林奕帆/all/P1353'
gro_file_dir  = '/home/irisubuntu/林奕帆/all/P2353'
...
with tf.Session() as sess:  
    ...
    try:
        while not coord.should_stop() and i<1:
            img, label = sess.run([ori_batch,gro_batch])
            
            for j in np.arange(BATCH_SIZE):
                print('lable: %d', ori_list[j])
                plt.imshow(img[j,:,:,:])
                plt.show()
            i+=1
...
with tf.Session() as sess:  
    ...
    try:  
        ...
        for j in np.arange(BATCH_SIZE):
            ...
            fname = tf.constant('/home/irisubuntu/林奕帆/all/P353/' + '%d原圖.jpg' %(j+1))
            fwrite = tf.write_file(fname, images_encode)
            result = sess.run(fwrite)

            images_encode2 = tf.image.encode_jpeg(gro[j, :, :, :]*255)
            fname2 = tf.constant('/home/irisubuntu/林奕帆/all/P353/' + '%d對應.jpg' %(j+1))
