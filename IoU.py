    ...
    for i in range(img_h):
        for j in range(img_w):
            ...
            if ans_data[i, j] > 200 and res_data[i, j] > 200:
                global both_num
                both_num = both_num + 1
    return ans_num, res_num, both_num, (both_num)/(ans_num + res_num - both_num)
...
for i in np.arange(len(a_list)):
    ans_t, res_t, both_t, iou_t = get_iou(a_list[i], r_list[i])
    iou.append(iou_t)
...
f = open(file_dir + "/iou.csv","w",newline='')
w = csv.writer(f)
